...
describe('会员等级人工调整', () => {
  function createService(totalConsumeAmount: number) {
    ...
    const benefits: any = { reissueForManualLevelChange: jest.fn() };
  ...
  it('累计有效消费不足60000时不能人工授予V6', async () => {
    ...
    await expect(service.adjustMemberLevel({ userId: 2, levelCode: 'V6', remark: '人工晋升', confirmBenefitReset: true }, 1))
      .rejects.toBeInstanceOf(BadRequestException);
  });

  it('达到条件后记录等级操作并按目标等级发放独立权益', async () => {
    const { service, tx, benefits } = createService(60000);
    tx.memberLevelOperation.create.mockResolvedValue({ id: 88 });
    await service.adjustMemberLevel({ userId: 2, levelCode: 'V6', remark: '条件已核验', confirmBenefitReset: true }, 1);
    ...
    expect(benefits.reissueForManualLevelChange).toHaveBeenCalledWith(expect.objectContaining({
      afterLevelCode: 'V6', userId: 2, sourceId: 88,
    }));
  });

  it('未确认权益重置时拒绝调整', async () => {
    const { service } = createService(60000);
    await expect(service.adjustMemberLevel({ userId: 2, levelCode: 'V6', remark: '条件已核验' }, 1))
