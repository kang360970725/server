...
- 页面权限只控制入口；按钮权限控制动作。后端 `UsersService` 仍会按目标用户类型做范围校验，角色要操作打手时必须同时拥有 `users:staff:page` 和对应打手按钮权限。打手管理顶部员工资金统计属于敏感汇总，只允许拥有 `users:staff:wallet-stats:button` 或 `SUPER_ADMIN` 展示和加载。
