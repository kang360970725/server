...
- 配置写入新订单的 `projectSnapshot`；迁移时也把上线时配置固化到全部存量订单快照，防止商品后续调价或改规则追溯影响已有订单。仅老快照缺少新字段时才兼容读取当前商品配置。
- 全额模式：所有存单轮正进度的基础收益归零，炸单负进度原值保留并回灌可分配池，最后完成轮获得订单结算池（含炸单回灌）扣平台抽成后的收益；末轮不再因默认中评把基础收益降到 65%，但明确的售后责任扣款/维护费仍执行。历史零收益参与人不能再被选择获得订单打赏。
- 最低进度模式：先推算末轮剩余进度，不足时从倒数第二个存单轮开始向前逐轮扣减正进度；一个轮次内多人按原正进度比例扣减。负进度不参与扣减。扣减后的有效进度统一进入原有抽成、评级、售后和钱包结算链路。
- 业务默认：新建的非体验保底商品最低进度为 800 万；体验商品按项目分别配置。迁移只用当时价格做一次存量初始化：体验 108 商品切全额模式，体验 199 商品设 600 万，非体验保底商品设 800 万。以后 108→118、199→218 或新增商品均不再依赖价格。
- 新增迁移 `20260901090000_add_guaranteed_settlement_policy`。隔离 MySQL 已验证字段、四类商品初始化及 JSON 数值/空值快照类型；完整空库迁移链仍会被既有 `20260601120000_add_order_source_and_customer_user` 对 `orders` 表名的引用阻断，与本迁移无关。
- 单元测试覆盖全额赢家、炸单回灌、600 万单轮倒扣、800 万跨多轮倒扣、快照优先级、商品默认值/冲突清理/非法配置。

---

## 2026-08-31 ｜派单对账按收款日统计与会员充值实收

- 对账和订单页今日营收统一使用北京时间收款日（半开日期区间），摘要与日明细包含一致的渠道范围。非小程序外部订单收款归收钱吧；会员储值消费单独列出，不重复计现金收入。
- 今日营收＝订单外部实收＋成功会员充值实收，均为退款前收款口径，不是利润/退款后净额。人工会员充值也计入收钱吧；微信充值独立归小程序支付，赠送余额不计入营收。微信充值优先取回调 amount.total（分），无回调金额时兼容 payAmount。
- 微信订单及充值成功日期优先使用通知 success_time，避免通知跨日到达造成日期偏移；人工收款仍使用操作时刻（创建订单可录入实际付款时间）。
- 新增 `OrderReceipt` 和迁移 `20260831140000_add_order_receipts`。已付款订单首次进行新补收时，将旧累计实收/原付款日固化为 INITIAL，增量另存 SUPPLEMENT；后续补收只进入补收当日。订单行锁＋同事务记录流水/累计金额，重复相同累计金额不重复入账。流水保留单号/渠道/派单人快照，无级联删除。
- 未产生新收款快照的历史订单仍按原订单记录读取；产生快照后排除原订单累计金额，避免双算。历史已发生但无完整时间记录的补收无法准确拆日，本次不猜测、不批量回补历史。统计采用 RepeatableRead 快照，避免并发提交时混读。
- 普通订单编辑禁止修改已付款订单实收金额和付款日期，须走补收/退款流程，避免直接改动历史日账。
- 前端订单首页新增会员充值实收并修正订单数字段；财务对账补充充值统计、收款类型、渠道、单号、交易金额/计入营收，派单人汇总明确不含充值，明细可横向滚动。
- 验证：新增针对北京时间边界、跨日、储值排除、充值赠送/待支付排除、支付通知日期、快照去重的单元测试；隔离 MySQL 验证并发补收、跨日读数、事务回滚、编辑拦截、充值本金入账。未读取或修改业务数据库，也未与真实收钱吧账单逐笔核对。
- 发布前须在目标环境应用新增迁移并生成 Prisma Client，然后更新后端与前端；本次仅在隔离测试库执行迁移，没有自动迁移业务库。

## 2026-08-31 ｜内部租号订单闭环及提现资金覆盖审核

### 同日体验与关联补齐
- 租号页面响应式适配：桌面结算采用分组面板，金额与说明并排，移动端纵向排列；创建/结算/废除弹窗在手机端全屏，表单独立滚动、底部操作固定。移动端租号订单及风控结果改为单列信息卡片，详情资金流水改为卡片，筛选/分页保留。样式限定在租号页面和弹窗，权限及资金计算不变。
- 响应式验证使用无数据库连接的模拟接口，覆盖桌面、390×844及320×740，检查结算开关、实时退补预览、风控精确查询、创建日期选择、详情及废除入口。修复窄屏栅格横向滚动及全局顶部操作区在租号页面的挤出问题。TypeScript与前端构建通过，未重跑与本次纯UI改动无关的资金集成测试。
- 创建编号改为后端随机生成 `LMSH` + 8位数字，前端不展示、不提交；数据库唯一索引排重，碰撞后回滚并重新生成（最多5次），资金流水冲突不触发自动重试。
- 创建弹窗顶部采用双列金额卡片，仅保留参考资产、本次实扣；资产和扣款口径不变。
- 钱包流水按租号 `sourceType + sourceId` 批量关联编号，兼容已有扣租金、押金、结算退款、补差、废除返还；支持订单编号搜索，保留本人/指定用户范围与普通订单筛选，不复用普通订单ID或回写历史账目。同步补齐租号流水的余额变动展示计算。
- 租号详情批量查询操作人姓名，展示创建/结算/废除人员姓名，无姓名或用户已不存在时显示“未知操作人”；保留原始操作人ID用于审计。
- 此次无需数据库结构变更。针对性测试70项通过；隔离MySQL验证编号碰撞重试、扣退原子性、按编号查询钱包流水及操作人姓名通过。
- 前后端TypeScript检查及前端开发构建通过。浏览器导航隔离测试页被拦截（ERR_BLOCKED_BY_CLIENT），本次未完成可视化验收。

### 业务与资金口径
- 新增独立 `RentalOrder` / `rental-orders` 模块，管理端接口隔离在 `/admin/rental-orders`；不复用服务订单、设备租赁账单或平台保证金。未来自助端需增加独立controller/DTO和本人身份约束，复用领域服务，不能开放管理端代扣接口。
- 风控查询操作栏增加快捷创建，自动关联当前服务者。流水号为LMSH加6—8位数字，数据库唯一；号源编号手填必填，号源ID为空，保存服务者名称和号源编号快照。来源暂为ADMIN。
- 创建实际扣除预扣租金R＋本单押金D，允许可用余额变负；授信资产只包括可用余额＋有效收益冻结，排除提现冻结和平台保证金。原租号风控参考金额同步排除平台保证金。
- 开始日期按上海时间计算：14:00:00时仍为今日，超过14:00:00顺延一天；强制结算日期不得早于开始日期，到期只提示/筛选，不自动结算。
- 实际费用＝R－租金退差F＋损耗L＋异常赔付C；净退款＝D＋F－L－C。净退款为正退可用，为负补扣可用（允许负余额），不重复退押金后再收费。
- 损耗、退差、异常赔付启用后按规则填写说明，金额最多两位小数；退差不得超过租金。号主结算金额仅登记统计，暂不自动出款。结算保存公式版本、金额快照、操作人和时间。
- 创建2小时内且未结算可废除，返还租金和押金全额，必须留原因，保留订单和冲正流水。
- 状态RUNNING/SETTLED/VOIDED；订单行锁、钱包行锁、版本校验、事务和来源唯一流水约束防重复扣退。结算/废除重复请求返回已有终态，不重新落账。
- 租号流水使用AVAILABLE及明确IN/OUT类型，兼容现有钱包重放的普通可用余额收支路径；备注关联LMSH编号。

### 提现规则
- 新申请提现仍只使用可用余额，首次提现20单规则保持不变。
- 已申请提现出款校验：提现预留流水完整、覆盖所有在途申请，收益冻结与wallet_holds匹配，分桶与汇总一致，且可用＋收益冻结≥0；平台保证金不计入覆盖。余额不足仍允许驳回释放预留资金。
- 共享资金检查用于租号创建与提现出款；历史分桶/流水不一致时明确拦截，不按虚高汇总余额放行。
- 审核、人工出款及成功回调路径改为READ COMMITTED，避免先读取提现申请后等待钱包锁时，沿用并发扣款前的旧快照。
- 审核弹窗增加非提现净资产、提现冻结、收益冻结及新的校验说明。

### 页面、权限及统计
- 新页面：财务管理→租号订单，支持查询、状态/逾期筛选、详情/关联钱包流水、结算和废除。
- 上海自然日统计：创建数量和租金（排除已废除、金额不含押金）；结算数量、服务者实际费用、号主结算金额按settledAt统计。顶部今日统计不随列表筛选改变。
- 独立权限：`rental-orders:page`，以及create/settle/void三个按钮权限。后端逐接口校验，不以余额查询权限代替资金操作权限。权限列表动态补齐及seed均已更新。
- 未来对外前端独立建设，优先Ant Design Mobile；本阶段不创建对外平台，也不拆分钱包后端。

### 迁移与上线注意
- 新迁移 `20260831090000_add_rental_orders`：租号订单表及索引、钱包业务枚举、四个钱包余额字段由Decimal(10,1)扩展至Decimal(12,2)，避免分币租金/退款被舍入。平台保证金字段不变。
- 已在临时本地MySQL从旧schema升级执行迁移，迁移后与当前schema比较无差异；没有对开发/生产业务库执行迁移。
- 正式上线需备份并在合适维护窗口执行迁移（余额字段/枚举ALTER可能涉及锁表），随后生成Prisma客户端并发布前后端。
- 已有角色需在权限管理加载新权限后，为商行工作人员分配租号页面及所需创建/结算/废除按钮权限；不要为此直接执行完整seed覆盖现有角色配置。新安装seed的商行角色已包含租号权限。
- 尚无号源库存、外部支付、自助下单、号主自动结算、自动到期扣费；这些为后续独立平台阶段功能。

### 验证
- 后端12组134项单元测试通过，覆盖金额/日期边界、押金抵扣、补差、废除、幂等、事务失败回滚、权限、无效收益冻结来源及提现资金完整性。修复此前设备租赁测试缺少事务/日志mock的问题，未改该模块业务实现。
- `test/rental-orders.mysql.integration.ts` 在严格限定的本地隔离测试库运行：真实钱包服务、提现服务与租号服务；验证并发超额创建、编号排重、重复退款、结算/废除竞争、分币精度、审计写入失败回滚、持锁等待后读新余额，以及统计和详情。
- Prisma validate/generate、后端完整build、前端build:dev及前端tsc --noEmit均通过。构建仅提示已有浏览器兼容数据库与Prisma配置弃用警告。
- Browser技能用于隔离UI测试：真实前端构建＋本地合成身份/辅助接口＋实际租号服务及临时MySQL；验证查询→创建→结算补差、废除退款和390px移动布局。未使用真实业务账号或真实资金，未完成生产登录环境验收。
