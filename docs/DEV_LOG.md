...
## 2026-08-31 ｜首次提现由接单天数改为接单单数

- 新增规则配置字段 `firstWithdrawMinAcceptedOrders`，默认20单，支持默认规则及分组规则配置非负整数；配置0表示关闭单量限制。
- 不再读取 `firstWithdrawMinAcceptedDays`。已有JSON规则未提供新字段时按20单生效，不把历史15天误转换为15单；保存配置后使用新字段，无需数据库结构迁移。
- 首次提现统计本人已确认接单（acceptedAt非空、rejectedAt为空）的不同订单数量；同一订单多轮接单只计一单，历史非活跃参与轮次仍计入，取消/退款订单不计入。接单即可计数，不要求结单。
- 余额门槛保持原实现：可用余额达到命中规则配置即可，默认≥1000；首次提现认定仍沿用无历史提现申请，后续提现和退店服务者豁免规则不变。
- 更新管理端基础配置、服务者规则摘要、退店预览及API类型，展示单位由天改为单。前后端需配套发布。
- 本次不调整已有提现审核负余额规则，不实现租号订单需求。
- 验证覆盖旧规则缺省20单、自定义门槛、0/19/20/21单、余额999/1000和非法单数；仅运行相关单元测试，未执行build或Prisma命令。
- 后续补充验证：增加满20/21单、自定义3单、0单关闭限制的成功提现路径，以及已有提现历史跳过首次门槛；断言冻结金额、提现单与流水关联，失败校验不调用资金写入。相关3组32项单元测试全部通过。
- 全部后端单元测试：8组57项中54通过、3失败。失败均在未改动的设备租赁测试中，分别缺少 `$transaction` 与 `userLog.create` mock；本次未修改该模块。前端4个改动文件通过TypeScript转译语法检查；尚未进行真实数据库集成或浏览器交互验证，未执行完整构建。
