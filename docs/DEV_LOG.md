...
- 新增 `offline_fee_contracts` 表，维护线下管理费配置：服务者、每月费用、开始月份、结束月份、启停状态和备注。
- 线下管理费每月 20 日自动生成当月账单，账单扣费时间固定为当月 20 日；管理端保留“生成月账单”按钮用于补生成或手动重试。
- 线下费用账单页取消“编辑账单”“催收”和“回退”入口，账单生成后仅保留手动缴费、其他渠道已缴、减免等结算动作。
- 提现前线下费用校验改为与设备租赁一致：只有已存在未结清账单，且进入账单到期前 24 小时内，才要求优先缴纳后再提现；未来未到期账单不提前限制提现。
- 线下费用账单废弃原“按业绩基数/费率生成月账单”的前端使用方式，改为后台手动录入账单：选择线下服务者、账单月份、扣费时间、扣费金额和备注。
...
- 线下费用列表新增统计口径：账单金额、收费累计（手动缴费 + 服务者提现补缴）、其他渠道收取、减免、未结清。
